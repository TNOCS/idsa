...
// TODO Parallel planning would be much faster.
...
public class AgendaPlanner {
    ...
    private static void addSuccess(PossibleActivity pa) { // TODO See above or remove.
    ...
    private static void addFailure(PossibleActivity pa) { // TODO See above or remove.
        Tuple<Integer, Integer> sf = successesAndFailures.get(pa);
        if (sf == null) {
            sf = new Tuple<>(0, 1);
            successesAndFailures.put(pa, sf);
    ...
    public void createAgendas(Set<Group> households) {
        ...
        double total = 19 * households.size(); // TODO Magic numbers that make the progress look a bit smoother.
        int current = 0;
        ...
        for (Group household : households) {
            ...
            ProgressNotifier.notifyProgress(factor(current++, total));
        }

        // Give an overview of the expected number of agents that will do each activity and the available capacity.
        // TODO Move to logger.
        Map<String, Double> overviewMultipliers = overviewMultipliers();
        TreeMap<String, Integer> overviewCapacities = overviewCapacities();
        ...
        for (Group household : households) {
            ...
            ProgressNotifier.notifyProgress(factor(current = current + 16, total));
        ...
        for (Group household : households) {
            ...
            ProgressNotifier.notifyProgress(factor(current = current + 2, total));
                        ...
                        if (agent.getAgenda() != null && agent.getAgenda().size() > 0) {
                            ...
                            if (agentsWithCommonActivities.size() > 1) {
                                ...
                                for (Agent a : agentsWithCommonActivities) {
                                    ...
                                    System.out.println(overviewAgenda(a));
                                }
                            } else {
                                System.out.println("\n\nAgent " + agent + " has no joint activities, showing only this agent's agenda.\n---");
                                System.out.println(overviewAgenda(agent));
                            }
                            if (--sampleSize == 0) {
                                break;
                            }
        ...
        // Show a summary of planned activities. This includes walking activities, the status report above does not.
    ...
    private Map<String, Double> overviewMultipliers() {
    ...
    private TreeMap<String, Integer> overviewCapacities() {
        ...
        for (Area a : world.getAreas()) {
            ...
            for (LocationFunction function : functions) {
                ...
                if (capacity > 0) {
                    ...
                    overviewCapacities_update(count, capacity, fClass);
                    ...
                    for (Class<? extends LocationFunction> superclass : superclasses) {
                        overviewCapacities_update(count, capacity, superclass);
                    }
                }
            }
    ...
    private void overviewCapacities_update(HashMap<Class<? extends LocationFunction>, Integer> count, int capacity, Class<? extends LocationFunction> fClass) {
    ...
    // A bit of a useless utility method.
    private static double factor(int current, double total) {
    ...
    private void applyMultipliers(Agent agent) {
        ...
        // normalize(agentProbabilities); // TODO Can we normalize the probabilities in some way?
        activityProbabilities.put(agent, agentProbabilities);
    }

    // TODO Query activities by class instead of class's simple name.
    private double getLikelihood(Map<String, Double> likelihoods, String activityName) {
        return likelihoods.containsKey(activityName) ? likelihoods.get(activityName) : 0.0;
    }
    ...
    // Indicate whether to keep the second activity
    private void optimizeActivityPair(List<Activity> agenda, int i) {

        Activity activity1 = agenda.get(i);
        Activity activity2 = agenda.get(i + 1);
        ...
        // Two moving activities can be combined into one.
            ...
            if (firstActivityCanBeModified && secondActivityCanBeModified) {
                ...
                agenda.set(i, new BasicMovementActivity(activity1.getPossibleActivity(), activity1.getStartLocation(), activity1.getStartTime(), activity2.getEndLocation(), newEndTime, activity1.getParticipants(), false));
                agenda.set(i + 1, null);
    ...
    private String overviewAgenda(Agent agent) {
        String s = "";
